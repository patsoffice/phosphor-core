...
fn test_neg_direct() {
    ...
    tick(&mut cpu, &mut bus, 4); // 1 fetch + 3 execute (rmw_direct)
...
fn test_tst_direct() {
    ...
    tick(&mut cpu, &mut bus, 3); // 1 fetch + 2 execute (alu_direct, read-only)
...
fn test_neg_extended() {
    ...
    tick(&mut cpu, &mut bus, 5); // 1 fetch + 4 execute (rmw_extended)
...
fn test_tst_extended() {
    ...
    tick(&mut cpu, &mut bus, 4); // 1 fetch + 3 execute (alu_extended, read-only)
...
fn test_lda_extended_sta_extended() {
    ...
    tick(&mut cpu, &mut bus, 4); // LDA extended
    assert_eq!(cpu.a, 0x77);
    tick(&mut cpu, &mut bus, 4); // STA extended
...
fn test_clr_direct_then_inc_direct() {
    ...
    tick(&mut cpu, &mut bus, 4); // CLR direct
    assert_eq!(bus.memory[0x0010], 0x00);
    tick(&mut cpu, &mut bus, 4); // INC direct
