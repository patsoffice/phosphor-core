...
fn test_add_accumulator_immediate() {
    ...
    assert_eq!(
        sys.get_cpu_state().a,
        0x30,
        "A should be 0x30 after 0x10 + 0x20"
    );
    assert_eq!(
        sys.get_cpu_state().cc & CcFlag::C as u8,
        0,
...
fn test_sub_accumulator_immediate() {
    ...
    assert_eq!(
        sys.get_cpu_state().a,
        0x20,
        "A should be 0x20 after 0x30 - 0x10"
    );
    assert_eq!(
        sys.get_cpu_state().cc & CcFlag::C as u8,
        0,
...
fn test_mul_carry_set() {
    ...
    assert_eq!(
        sys2.get_cpu_state().cc & CcFlag::C as u8,
        CcFlag::C as u8,
        "Carry should be set (bit 7 of B is 1)"
    );
    assert_eq!(
        sys2.get_cpu_state().cc & CcFlag::Z as u8,
        0,
        "Zero should be clear"
    );
}
