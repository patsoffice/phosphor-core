...
fn test_increment() {
    ...
    sys.tick(); sys.tick(); // LDA #$7F

    // INCA: 0x7F + 1 = 0x80 (signed overflow: positive -> negative)
    sys.tick(); sys.tick();
    let state = sys.get_cpu_state();
    assert_eq!(state.a, 0x80);
    assert_eq!(state.cc & (CcFlag::V as u8), CcFlag::V as u8, "Overflow should be set (0x7F -> 0x80)");
    assert_eq!(state.cc & (CcFlag::N as u8), CcFlag::N as u8, "Negative should be set");
    assert_eq!(state.cc & (CcFlag::Z as u8), 0);

    sys.tick(); sys.tick(); // LDB #$FF

    // INCB: 0xFF + 1 = 0x00 (wraps to zero)
    sys.tick(); sys.tick();
    let state = sys.get_cpu_state();
    assert_eq!(state.b, 0x00);
    assert_eq!(state.cc & (CcFlag::Z as u8), CcFlag::Z as u8, "Zero should be set");
    assert_eq!(state.cc & (CcFlag::N as u8), 0);
    assert_eq!(state.cc & (CcFlag::V as u8), 0, "Overflow should be clear (0xFF -> 0x00 is not signed overflow)");
}
...
fn test_decrement() {
    ...
    assert_eq!(state.cc & (CcFlag::V as u8), CcFlag::V as u8, "Overflow should be set (0x80 -> 0x7F)");
    assert_eq!(state.cc & (CcFlag::N as u8), 0, "Negative should be clear");
    assert_eq!(state.cc & (CcFlag::Z as u8), 0);

    sys.tick(); sys.tick(); // LDB #$01
