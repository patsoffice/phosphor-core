...
fn test_beq_taken() {
    ...
    sys.tick(); sys.tick(); sys.tick(); // BEQ
...
fn test_beq_not_taken() {
    ...
    sys.tick(); sys.tick(); sys.tick(); // BEQ (not taken)
    assert_eq!(sys.get_cpu_state().pc, 0x04); // 0x04 + 0 (not taken) -> 0x04

    sys.tick(); sys.tick(); // Next instruction (LDA #$42)
    assert_eq!(sys.get_cpu_state().a, 0x42);
}
...
fn test_bne_taken() {
    ...
    sys.tick(); sys.tick(); // LDA
    sys.tick(); sys.tick(); sys.tick(); // BNE
...
fn test_bmi_taken() {
    ...
    sys.tick(); sys.tick(); sys.tick(); // BMI
    assert_eq!(sys.get_cpu_state().pc, 0x06);
}
